"""Integration tests for the Bayt.com connector (mocked Playwright)."""
...
class TestBaytScrape:
    ...
    async def test_safe_scrape_handles_exception(self, connector):
        with patch.object(connector, "scrape", side_effect=Exception("Scrape failed")):
            result = await connector._safe_scrape()
        assert result == []

    @pytest.mark.asyncio
    async def test_empty_page_returns_empty_list(self, connector):
        """When Playwright returns no cards, connector returns empty list."""
        with patch("src.connectors.bayt.async_playwright") as mock_pw:
...
class TestBaytJobNormalization:
    def test_normalize_adds_source(self, connector):
        raw = {"title": "Director", "company": "C", "location": "Dubai", "url": "https://bayt.com/job/1"}
        normalized = connector._normalize_job(raw)
        assert normalized["source"] == "bayt"

    def test_normalize_adds_missing_keys(self, connector):
        raw = {"title": "VP", "url": "https://bayt.com/job/2"}
        normalized = connector._normalize_job(raw)
