"""Integration tests for the Naukrigulf connector (mocked Playwright)."""
...
def connector(config):
    return NaukriGulfConnector(config=config)


class TestNaukriGulfBasics:
    def test_name_is_naukrigulf(self, connector):
        assert connector.name == "naukrigulf"

    def test_has_search_urls(self, connector):
        assert hasattr(connector, "SEARCH_URLS")
        assert len(connector.SEARCH_URLS) > 0

    def test_search_urls_target_naukrigulf(self, connector):
        urls = " ".join(connector.SEARCH_URLS).lower()
        assert "naukrigulf" in urls

    def test_rate_limiter_configured(self, connector):
        assert connector.rate_limiter is not None


class TestNaukriGulfScrape:
    @pytest.mark.asyncio
    ...
    async def test_empty_page_returns_empty_list(self, connector):
        with patch("src.connectors.naukrigulf.async_playwright") as mock_pw:
    ...
    def test_has_dismiss_popups(self, connector):
        """NaukriGulf has popup dismissal."""
        assert hasattr(connector, "_dismiss_popups")


class TestNaukriGulfNormalization:
    def test_normalize_adds_source(self, connector):
        raw = {
            "title": "Director Engineering", "company": "C", "location": "Dubai",
            "url": "https://www.naukrigulf.com/director-engineering-jobs-1"
        }
        normalized = connector._normalize_job(raw)
        assert normalized["source"] == "naukrigulf"
